...
class MainActivity : AppCompatActivity(), FileAdapter.OnFileClickListener {
    ...
    private fun displayFiles(path: String) {
        ...
        if (files != null) {
            val fileList =
                files.toList().sortedWith(compareBy({ !it.isDirectory },
                    { it.name.lowercase() }))
            adapter = FileAdapter(this, fileList)
            adapter.listener = this
            binding.rvFiles.adapter = adapter
        } else {
            Toast.makeText(this, "Cannot access path",
                Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun createNewFolder() {
        ...
        builder.setPositiveButton("Create") { _, _ ->
            ...
            if (!newFolder.exists()) {
                ...
                Toast.makeText(this, "Folder already exists",
                    Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun pasteFile() {
        copiedFile?.let {
            ...
            try {
                ...
                displayFiles(currentPath)
                Toast.makeText(this, "File pasted",
                    Toast.LENGTH_SHORT).show()
            } catch (e: IOException) {
                e.printStackTrace()
                Toast.makeText(this, "Paste failed: ${e.message}",
    ...
    private fun copyFile(file: File) {
        copiedFile = file
        invalidateOptionsMenu() // This will call onPrepareOptionsMenu to
        //show/hide the paste button
        Toast.makeText(this, "Copied ${file.name}",
            Toast.LENGTH_SHORT).show()
    }
}
